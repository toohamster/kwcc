...
### 文件读取原则
- **必须一次性 Read 完整文件**，不能几行几行地反复读同一个文件
- 一个文件读几分钟是极其浪费时间的行为
- 如果需要参考文件内容，先一次性读完整文件再做操作

### 文件编辑原则
- **同一文件的多次修改必须一次性完成**，用 Write/Edit 一次性改完再让用户确认
- **绝对禁止**一行一行、一处一处地让用户反复确认同一个文件的修改
- 这是非常耗时间且低级的做法
- 只有在涉及**不同文件**的独立修改时，才需要分别确认
