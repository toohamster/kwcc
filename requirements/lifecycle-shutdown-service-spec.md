...
/* 注册约定：entry 中的函数指针（dirty_count / shutdown）和 entry 本身必须是静态存储期，
 * 不得是栈上临时变量。服务不复制函数指针的指向目标，只保存指针值。 */
...
1. **force_all 直接调用各模块的 shutdown(1)，不走 bus**。原因：force_all 可能在 kill 信号处理中被调用，此时 bus 可能处于不可用状态（模块正在被 unload），走 bus 会失败且无报错。运行时的 poll 走 bus 是因为需要解耦触发源和执行方，退出时没有这个需求——调用方明确知道要做什么。

2. **shutdown(1) 只能由 force_all 调用，运行期禁止直接调 shutdown(1)**。原因：shutdown(1) 会强制终止正常请求（子进程还在跑、数据还没读完），运行期误调会导致正常请求被中断、JS 侧 Promise 异常。运行期只走 shutdown(force=0)（轻量回收）。如果模块的 shutdown(1) 被运行期直接调用，应打 `log_warn` 警告并跳过。

3. **force_all 的调用时机**：main.m cleanup()（正常退出）+ kill 信号处理函数（SIGINT/SIGTERM）。

```
1. 逆序遍历注册表
2. 对每个 entry：
   a. 逆序调 entry->shutdown(1)    // 不走 bus
   b. log_info 记录结果
3. 服务自身标记为 shutdown
...
/* cleanup() */
kwcc_lifecycle_shutdown_force_all();  // 逆序调各模块 shutdown(1)，不走 bus
