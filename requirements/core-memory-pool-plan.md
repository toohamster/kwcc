...
>
> 本执行计划已被 `mempool-design.md`（v7）取代。
>
> ## 为什么不用旧计划
>
> 旧计划（本文件）基于 `core-memory-pool.md` 的 Core/App/User 三池架构，核心问题：
>
> 1. **池架构僵化** — 3 个业务池固定大小，JS 启动时指定后无法动态扩容，预估不准浪费或不够用。新方案 L0-L7 按数据长度分层，每种池独立扩缩容，满即扩
> 2. **无多池管理** — 每域只 1 个池，满即返回 NULL。新方案每种池类型可展开多个实例（L0 最多 16 池），通过 key_map 统一管理
> 3. **查找慢** — 线性扫描 O(n)。新方案 32768 条目 hash 表 O(1) 定位
> 4. **无 TLV** — 对象数据纯字符串存储。新方案 TLV 二进制省 30% 空间 + 路径查询
> 5. **无优化** — 无常量表、无自动路由、无类型区分
> 6. **JS API 落后** — setApp/setUser/setCore 语义模糊，类型不区分。新方案 appSetInt/String/Bool/Json/Tlv 扁平按类型，基础类型自动走常量表
> 7. **两阶段初始化复杂** — C 层建 Core + JS 层建 App/User。新方案 L0-L7 统一初始化一步到位
>
> 新旧方案完整对比见 `core-memory-pool.md` 顶部的对比表。
>
> **新的 7 Phase 执行计划见 `mempool-design.md` 末尾。**
>
