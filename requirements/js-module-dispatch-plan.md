...
#define KWCC_JS_MAX_APIS     32   /* 所有模块 API 总数上限 */
...
static void kwcc_js_dispatch_add(const char *module, const char *func, kwcc_js_handler_t handler) {
    if (g_kwcc_js_dispatch_count >= KWCC_JS_MAX_APIS) {
        log_error("js: dispatch table full (%d)", KWCC_JS_MAX_APIS);
        return;
    }
...
### 4. kwcc_js_call — 新的 JS 全局函数

替代 `kwcc_js_mquickjs_call`，注册为 JS 全局函数 `kwcc_js_call`。

```c
/* 新全局函数：kwcc_js_call(module, func, ...args) */
JSValue kwcc_js_call(JSContext *ctx, JSValue *this_val, int argc, JSValue *argv) {
    (void)this_val;
...
在 `mqjs_stdlib.c` 中将 `kwcc_js_mquickjs_call` 替换为 `kwcc_js_call`：

```c
// 旧
JS_CFUNC_DEF("kwcc_js_mquickjs_call", -1, kwcc_js_mquickjs_call),
// 新
JS_CFUNC_DEF("kwcc_js_call", -1, kwcc_js_call),
```
...
**`$config` 的 12 个 handler 不处理**：它们通过 `mqjs_stdlib.c` 的 `JS_CFUNC_DEF` 直接注册为 JS 全局函数，不走分发表也不走 `kwcc_js_call`，保持现状不动。

**核心决策：core handler 签名暂不改**。现有 handler 使用 mquickjs 签名（`JSContext*`/`JSValue*`/`this_val`），本计划只为分发机制服务。core handler 通过**适配器包装**注册进分发表：

```c
/* core handler 适配器：把 ops 签名转为 mquickjs 签名，调原始函数 */
static kwcc_js_val_t js_core_mempool_dump_stats_adapter(kwcc_js_ops_t *ops, int argc, kwcc_js_val_t *argv) {
    return kwcc_js_mempool_dump_stats((JSContext *)ops->ctx, NULL, argc, argv);
}

static kwcc_js_val_t js_core_mempool_dump_all_adapter(kwcc_js_ops_t *ops, int argc, kwcc_js_val_t *argv) {
    return kwcc_js_mempool_dump_all((JSContext *)ops->ctx, NULL, argc, argv);
}

/* core API 列表（只有 mempool，config 不走分发表） */
static const kwcc_js_api_t g_kwcc_js_core_apis[] = {
    { "mempool_dump_stats", js_core_mempool_dump_stats_adapter },
    { "mempool_dump_all",   js_core_mempool_dump_all_adapter },
    { NULL, NULL }
};
...
**`$config` 处理方式**：12 个 config handler 通过 `mqjs_stdlib.c` 的 `JS_CFUNC_DEF` 直接注册为 JS 全局函数，不走分发表也不走 `kwcc_js_call`，保持现状不动。
...
| `kwcc_js_mquickjs_call()` | 被 `kwcc_js_call()` 替代 |
...
// 新
kwcc_js_call("core", "mempool_dump_stats")
kwcc_js_call("core", "mempool_dump_all", p, s)
```
...
1. 声明改为 `JSValue kwcc_js_call(JSContext *ctx, JSValue *this_val, int argc, JSValue *argv);`
2. 注册改为 `JS_CFUNC_DEF("kwcc_js_call", -1, kwcc_js_call),`
...
5. 实现 `kwcc_js_call` 函数（使用 `ops->to_cstring` 提取 module/func，调 `kwcc_js_dispatch_call`）
6. 保留 `kwcc_js_mquickjs_call` 作为兼容包装（内部调 `kwcc_js_call`），避免 Stage 1 重建前编译断
7. 保留 `g_kwcc_js_cfun_handlers[]` 代理表（Step 4 再删），确保中间态编译通过

**中间态说明**：此时存在两套并行机制——旧的代理表 + 新的分发表。`kwcc_js_mquickjs_call` 走代理表（旧路径），`kwcc_js_call` 走分发表（新路径）。JS 端仍调用 `kwcc_js_mquickjs_call`，功能不变。
...
1. `deps/mquickjs/mqjs_stdlib.c`：
   - 声明从 `kwcc_js_mquickjs_call` 改为 `kwcc_js_call`
   - 注册从 `JS_CFUNC_DEF("kwcc_js_mquickjs_call", -1, kwcc_js_mquickjs_call)` 改为 `JS_CFUNC_DEF("kwcc_js_call", -1, kwcc_js_call)`
...
3. `kwcc_js.c` JS 字符串迁移（和 Stage 1 重建同步，否则运行时报错）：
   - `kwcc_register_config_js` 中：`kwcc_js_mquickjs_call('kwcc_js_mempool_dump_stats')` → `kwcc_js_call("core", "mempool_dump_stats")`
   - `kwcc_register_config_js` 中：`kwcc_js_mquickjs_call('kwcc_js_mempool_dump_all', p, s)` → `kwcc_js_call("core", "mempool_dump_all", p, s)`
...
1. `tests/test_js_ops.c` 中如有引用 `kwcc_js_mquickjs_call` 的测试，改为 `kwcc_js_call`
2. 新增 `kwcc_js_dispatch_call` 分发机制测试：
   - 注册一个测试模块的 API，通过 `kwcc_js_call` 从 JS 端调用，验证分发正确
...
> 前置依赖：Part 1 完成（分发机制 + `kwcc_js_call` 已上线）
...
- [ ] `make run` 原有功能不受影响
- [ ] `kwcc_js_call("core", "mempool_dump_stats")` 在 JS 端可正常调用
...
| `kwcc_js_ops_t` | 不变 |
| 代理表 | 删除，由分发表替代 |
| `kwcc_js_mquickjs_call` | 删除，由 `kwcc_js_call` 替代 |
...
| JS 调用点遗漏 | 某处 `kwcc_js_mquickjs_call` 未迁移到 `kwcc_js_call` | grep 全局搜索确保无遗漏 |
