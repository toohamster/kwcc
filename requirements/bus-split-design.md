...
各模块是 bus 的 consumer：
  ...
  ├─ UI 模块：register_topic(topic, LIGHT) + emit
...
│  点击 → kwcc_bus_register_topic(topic, LIGHT)                     │
...
│  register_topic / subscribe / unsubscribe / emit                          │
├──────────────────────────────────────────────────────────┤
...
```
UI 每帧绘制 → kwcc_bus_register_topic("ui/calc/btn0", KWCC_BUS_MODE_LIGHT)
  → 如果 topic 已存在 → 覆盖（bindOnce）
  → 如果 topic 不存在 → 新占位

UI 点击 → kwcc_bus_emit("ui/calc/btn0", "click", 5)
  → bus 查 topic 属性 → KWCC_BUS_MODE_LIGHT
  → 查 LIGHT map 确认有效 → 走 NORMAL 链路触发 subscriber
```

特点：
- **bindOnce**：同一个 topic 多次 register 只是覆盖，不重复占位
- 无 ref_count，无生命周期管理
- emit 时精确匹配 topic，确认后走 NORMAL 链路
- 每帧可重新 register，widget ID 变化不影响

### 两种模式的关系

LIGHT 和 NORMAL 是**独立的两条路径**，由 topic 属性决定：

```
kwcc_bus_emit("topic", data, len):
  1. 查 topic 属性 → LIGHT → 查 map 确认有效 → 继续
  2. 查 topic 属性 → NORMAL → 直接走第 2 步
  3. 遍历 NORMAL subscriber 链表 → 匹配 topic → 调 callback
```

两条路径互不干扰：
- LIGHT 只负责"声明这个 topic 有发射点"
- NORMAL 负责完整的 subscriber + callback 管理
- emit 时自动根据 topic 属性选择路径
...
/* ── 注册 topic 属性 ── */
/* 声明 topic 的模式，emit 时自动选择对应路径 */
void kwcc_bus_register_topic(const char *topic, kwcc_bus_mode_t mode);
...
/* ── 发射事件（自动根据 topic 属性选择 LIGHT/NORMAL 路径）─ */
...
```c
typedef enum {
    KWCC_BUS_MODE_LIGHT,    /* 轻绑定：每帧刷新，bindOnce 语义 */
    KWCC_BUS_MODE_NORMAL,   /* 真订阅：ref_count + 生命周期管理 */
} kwcc_bus_mode_t;
...
/* LIGHT topic 属性 map 节点 */
typedef struct {
    const char       *topic;      /* 精确 topic 字符串 */
    kwcc_bus_mode_t   mode;       /* topic 模式 */
    int               in_use;
} kwcc_bus_topic_t;
```
...
/* ── topic 属性 map ── */
#define KWCC_BUS_TOPIC_MAX 256
static kwcc_bus_topic_t  g_kwcc_bus_topics[KWCC_BUS_TOPIC_MAX];

/* ── 初始化 ── */
void kwcc_bus_init(void) {
    g_kwcc_bus_head = NULL;
    g_kwcc_bus_next_id = 1;
    memset(g_kwcc_bus_topics, 0, sizeof(g_kwcc_bus_topics));
}
...
void kwcc_bus_register_topic(const char *topic, kwcc_bus_mode_t mode) {
    ...
    for (int i = 0; i < KWCC_BUS_TOPIC_MAX; i++) {
        if (g_kwcc_bus_topics[i].in_use &&
            strcmp(g_kwcc_bus_topics[i].topic, topic) == 0) {
            g_kwcc_bus_topics[i].mode = mode;
            return;
        }
    }
    /* 2. 找空位，新增 */
    for (int i = 0; i < KWCC_BUS_TOPIC_MAX; i++) {
        if (!g_kwcc_bus_topics[i].in_use) {
            g_kwcc_bus_topics[i].topic = topic;
            g_kwcc_bus_topics[i].mode = mode;
            g_kwcc_bus_topics[i].in_use = 1;
            return;
        }
    }
    log_warn("bus: topic table full (max=%d)", KWCC_BUS_TOPIC_MAX);
}

/* ── 查 topic 模式 ── */
static kwcc_bus_mode_t kwcc_bus_get_mode(const char *topic) {
    for (int i = 0; i < KWCC_BUS_TOPIC_MAX; i++) {
        if (g_kwcc_bus_topics[i].in_use &&
            strcmp(g_kwcc_bus_topics[i].topic, topic) == 0) {
            return g_kwcc_bus_topics[i].mode;
        }
    }
    return KWCC_BUS_MODE_NORMAL;  /* 默认 NORMAL */
}
...
/* ── 发射事件（自动根据 topic 属性选择路径）─ */
void kwcc_bus_emit(const char *topic, const void *data, size_t len) {
    if (!topic) return;

    /* 1. 查 topic 属性 */
    kwcc_bus_mode_t mode = kwcc_bus_get_mode(topic);

    /* 2. LIGHT 路径：确认 topic 有效，继续走 NORMAL 链路 */
    if (mode == KWCC_BUS_MODE_LIGHT) {
        /* LIGHT: topic 已在 register_topic 时声明，确认有效 */
        /* 继续走下面的 NORMAL 路径触发 subscriber */
    }

    /* 3. NORMAL 路径：遍历 subscriber 链表 */
    for (kwcc_bus_sub_t *sub = g_kwcc_bus_head; sub; sub = sub->next) {
...
LIGHT 模式不注册回调，它只声明"这个 topic 是 UI 发射点"。实际的回调触发走 NORMAL 路径：

```
UI 层每帧：
  kwcc_bus_register_topic("ui/calc/btn0", KWCC_BUS_MODE_LIGHT)
  → topic map 标记 "ui/calc/btn0" 模式为 LIGHT（bindOnce）
...
UI 点击时：
  ...
  → 查 topic 属性 → KWCC_BUS_MODE_LIGHT → 确认有效
  → 走 NORMAL 路径：遍历 subscriber → "*" 匹配 → 调 kwcc_js_on_bus_event
```

LIGHT 的作用是：
1. 声明 topic 模式，emit 时自动选择路径
2. bindOnce 语义，每帧重复 register 不重复占位
3. 每帧可重新 register，widget ID 变化不影响
...
void kwcc_ui_bind_topic(int id, const char *topic) {
    if (g_kwcc_ui_topic_count < KWCC_UI_TOPIC_MAP_SIZE && topic) {
        ...
        /* 注册 topic 属性为 LIGHT 模式 — bindOnce */
        kwcc_bus_register_topic(topic, KWCC_BUS_MODE_LIGHT);
    }
}
...
新:
  ui.button("OK", "test/click")
    ...
    → kwcc_bus_register_topic("test/click", KWCC_BUS_MODE_LIGHT)    /* bus LIGHT */
...
| 文件 | 改动 |
|------|------|
| `src/kwcc_bus.h` | 重写：去掉 `kwcc_dispatch_event`，加 `register_topic/subscribe/unsubscribe/emit` |
| `src/kwcc_bus.c` | 重写：topic 属性 map + NORMAL 链表 + ref_count，移除 mquickjs include，移除 topic map |
...
| 2 | kwcc_bus.c 重写为 topic 属性 map + NORMAL 链表 | `src/kwcc_bus.c` |
| 3 | kwcc_ui.c 加 topic map + dispatch_by_id + register_topic | `src/kwcc_ui.c` |
