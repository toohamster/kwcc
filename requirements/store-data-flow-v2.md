...
```c
void kwcc_begin_frame() {
    g_topic_map_count = 0;  // 清零，本帧从 0 开始覆盖
    mu_begin(&g_mu);
}
```
...
**为什么可行**：
- microui 同一帧内控件绘制顺序完全固定
- 256 槽位只需满足「单帧同屏可见控件数 < 256」
- 不存在跨帧清理、淘汰、溢出问题
- 零内存分配，纯覆盖写入
...
$bus.on(TOPIC.MAIN_OPEN_DIALOG, function(action, data) {
    $store.dispatch("dialog", "openDialog", "你已完成操作");
});
...
## 十一、Store 运行时实现（runtime/store.js）

```javascript
// createStore — 双参数 dispatch，零字符串解析
...
$store.dispatch("calc", "pressButton", data.label);
$store.dispatch("dialog", "closeDialog");
$store.dispatch("calc", "toggleHistory");  // 无 payload
```

**优势**：
- 零字符串解析（不用 split/slice 解析 `"calc/pressButton"`）
- 模块名 + 动作名显式传参，意图清晰
- mquickjs 下无额外性能开销
